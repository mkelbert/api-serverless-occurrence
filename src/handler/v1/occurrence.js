...
module.exports.register = (event, context, callback) => {
  ...
  .then(findUser => new Promise((resolve, reject) => {
    if(!findUser.email) return util.sendLambdaResponse(403, undefined, callback);
...
module.exports.delete = (event, context, callback) => {
  ...
  if(!args.code || !authenticator) return util.sendLambdaResponse(400, undefined, callback);
  ...
  .then(findOccurrence => new Promise((resolve, reject) => {
    if(!findOccurrence.email) return util.sendLambdaResponse(404, undefined, callback);
    if(findOccurrence.email != user.email) return util.sendLambdaResponse(401, undefined, callback);
    resolve();
  }))
  .then(() => occurrenceDao.delete(occurrence))
  .then(() => {
    return util.sendLambdaResponse(200, undefined, callback)
  })
  .catch(err => {
    console.log(err)
    return util.sendLambdaResponse(500, undefined, callback);
...
module.exports.list = (event, context, callback) => {
  ...
  if(!authenticator) return util.sendLambdaResponse(400, undefined, callback);
